...
def IngestData(directory):
    ...
    stats = (
        stats.groupby(["Station_ID", df["Date"].map(str).str[:4]])
        .agg({"Maxtemp": "mean", "Mintemp": "mean", "Precipitation": "sum"})
        .reset_index()
    )
    ...
    df.to_sql("app_weather", conn, if_exists="replace", index=True, index_label="id")
    stats.to_sql("app_stats", conn, if_exists="replace", index=True, index_label="id")
